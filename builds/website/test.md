...
| 7 | Lighthouse performance is strong (note score) | **Not run — now wired** | 2026-07-23: `@netlify/plugin-lighthouse` added to `netlify.toml`, auditing `/`, `/builds/`, `/tools/`, `/builds/website/` on every deploy. No thresholds set until a baseline exists. Still **no score to claim** until a deploy has run — record the real numbers here then. |
